           ...
           <div className="flex -space-x-6 items-center p-1">
               {displayedMembers.length > 0 ? (
                 displayedMembers.map((m, i) => (
                   <div key={i} className="relative z-0 hover:z-10 hover:scale-110 transition-all duration-300">
                     <div className="w-12 h-12 rounded-full overflow-hidden border-2 border-dark-card bg-dark-card ring-2 ring-dark-card">
                       {m.avatarUrl ? (
                          <img src={m.avatarUrl} alt={m.username} className="w-full h-full object-cover" />
                       ) : (
                          <div className="w-full h-full flex items-center justify-center bg-gray-800 text-gray-400 font-bold text-[10px]">
                 ...
                 <div className="relative">
                    <div className="w-12 h-12 rounded-full bg-gray-800 border-2 border-dark-card ring-2 ring-dark-card flex items-center justify-center">
                      <Icons.User className="w-5 h-5 text-gray-500" />
                    </div>
                 </div>
               )}
               {remainingMembers > 0 && (
                 <div className="relative z-0 hover:z-10 hover:scale-110 transition-all duration-300">
                    <div className="w-12 h-12 rounded-full bg-gray-800 text-white border-2 border-dark-card ring-2 ring-dark-card flex items-center justify-center text-xs font-bold">
...
// ... ManageRequestsModal (Unchanged) ...
const ManageRequestsModal: React.FC<{ collectionId: string; onClose: () => void }> = ({ collectionId, onClose }) => {
  const [requests, setRequests] = useState<CollectionRequest[]>([]);
  const [loading, setLoading] = useState(true);

  useEffect(() => { loadRequests(); }, [collectionId]);
  ...
  const handleAction = async (requestId: string, action: 'approve' | 'deny') => {
    try {
      if (action === 'approve') await dataService.approveRequest(requestId);
      else await dataService.denyRequest(requestId);
      setRequests(prev => prev.filter(r => r.id !== requestId));
    } catch (e) { console.error(e); alert(`Failed to ${action} request`); }
  };
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4 animate-in fade-in zoom-in-95">
      <Card className="w-full max-w-md p-0 max-h-[80vh] flex flex-col">
        ...
        <div className="overflow-y-auto p-6 flex-1">
          {loading ? ( <div className="flex justify-center p-8"><Icons.RefreshCw className="w-6 h-6 animate-spin text-neon-cyan" /></div> ) 
          : requests.length === 0 ? (
              ...
              {requests.map(req => (
                <div key={req.id} className="flex flex-col sm:flex-row items-center justify-between p-3 bg-white/5 rounded-lg border border-dark-border gap-4">
                  ...
                  <div className="flex flex-wrap items-center gap-4 w-full sm:w-auto justify-end">
                    <Button variant="outline" size="sm" className="pr-2 bg-green-500/10 hover:bg-green-500/20 text-green-500 border-green-500/20 h-8" onClick={() => handleAction(req.id, 'approve')}>Accept <Kbd>⏎</Kbd></Button>
                    <Button variant="outline" size="sm" className="pr-2 bg-red-500/10 hover:bg-red-500/20 text-red-500 border-red-500/20 h-8" onClick={() => handleAction(req.id, 'deny')}>Cancel <Kbd>Esc</Kbd></Button>
                  </div>
                </div>
              ))}
            </div>
...
export const CreateCollectionModal: React.FC<{ onClose: () => void; onCreated: (newId: string) => void; userId: string }> = ({ onClose, onCreated, userId }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      onCreated(newId);
      onClose();
    } catch (e) { console.error(e); alert("Failed to create"); } finally { setLoading(false); }
  };
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <Field>
            ...
            <div className="flex items-center gap-2 mt-1">
              <button type="button" onClick={() => setIsPublic(true)} className={`flex-1 px-3 py-2 rounded-lg border text-xs font-bold ${isPublic ? 'bg-neon-cyan/20 text-neon-cyan border-neon-cyan' : 'border-dark-border text-gray-500'}`}><Icons.Globe className="w-4 h-4 inline mr-2" /> Public</button>
              <button type="button" onClick={() => setIsPublic(false)} className={`flex-1 px-3 py-2 rounded-lg border text-xs font-bold ${!isPublic ? 'bg-neon-pink/20 text-neon-pink border-neon-pink' : 'border-dark-border text-gray-500'}`}><Icons.Lock className="w-4 h-4 inline mr-2" /> Private</button>
            </div>
            <FieldDescription>{isPublic ? "Anyone can remix." : "Only visible to you."}</FieldDescription>
          </Field>
          <Field>
            <FieldLabel>Style Tags</FieldLabel>
            <div className="flex gap-2 mt-1">
              <Input value={currentTag} onChange={e => setCurrentTag(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && handleAddTag(e)} placeholder="Add a tag..." />
              <Button type="button" onClick={handleAddTag} variant="secondary">Add</Button>
            </div>
            {tags.length > 0 && <div className="flex flex-wrap gap-2 mt-3 p-3 bg-black/20 rounded-lg">{tags.map(t => <span key={t} className="text-xs bg-neon-cyan/10 text-neon-cyan border border-neon-cyan/20 px-2 py-1 rounded-full">#{t} <button type="button" onClick={() => removeTag(t)}><Icons.X className="w-3 h-3 inline" /></button></span>)}</div>}
          </Field>
          <div className="pt-4"><Button type="submit" disabled={loading} className="w-full">{loading ? <Icons.RefreshCw className="animate-spin" /> : "Create Collection"}</Button></div>
        </form>
      </Card>
    </div>
  );
...
const EditCollectionModal: React.FC<{ collection: Collection; onClose: () => void; onUpdated: () => void }> = ({ collection, onClose, onUpdated }) => {
    ...
    const handleAddTag = (e?: React.FormEvent) => { e?.preventDefault(); if (currentTag.trim() && !tags.includes(currentTag.trim())) { setTags([...tags, currentTag.trim()]); setCurrentTag(""); } };
    const removeTag = (t: string) => setTags(tags.filter(tag => tag !== t));
    const handleSubmit = async (e: React.FormEvent) => {
      e.preventDefault(); setLoading(true);
      try { await dataService.updateCollection(collection.id, { name, description: desc, tags, isPublic }); onUpdated(); onClose(); } catch (e) { console.error(e); alert("Failed to update"); } finally { setLoading(false); }
    };
    ...
    return (
      <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4 animate-in fade-in zoom-in-95">
        <Card className="w-full max-w-md p-6 max-h-[90vh] overflow-y-auto">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <Field><FieldLabel>Name</FieldLabel><Input value={name} onChange={e => setName(e.target.value)} required /></Field>
            <Field><FieldLabel>Description</FieldLabel><Textarea value={desc} onChange={e => setDesc(e.target.value)} /></Field>
            <Field>
            <FieldLabel>Visibility</FieldLabel>
            <div className="flex items-center gap-2 mt-1">
              <button type="button" onClick={() => setIsPublic(true)} className={`flex-1 px-3 py-2 rounded-lg border text-xs font-bold ${isPublic ? 'bg-neon-cyan/20 text-neon-cyan border-neon-cyan' : 'border-dark-border text-gray-500'}`}><Icons.Globe className="w-4 h-4 inline mr-2" /> Public</button>
            ...
            <Field>
              <FieldLabel>Style Tags</FieldLabel>
              <div className="flex gap-2 mt-1"><Input value={currentTag} onChange={e => setCurrentTag(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && handleAddTag(e)} /><Button type="button" onClick={handleAddTag} variant="secondary">Add</Button></div>
              <div className="flex flex-wrap gap-2 mt-3">{tags.map(t => <span key={t} className="text-xs bg-white/10 px-2 py-1 rounded-full">#{t} <button type="button" onClick={() => removeTag(t)}><Icons.X className="w-3 h-3 inline" /></button></span>)}</div>
            </Field>
            <div className="pt-4"><Button type="submit" disabled={loading} className="w-full">{loading ? <Icons.RefreshCw className="animate-spin" /> : "Save Changes"}</Button></div>
          </form>
        </Card>
      </div>
    );
...
const CollectionDetailView: React.FC<{ 
    ...
    const remainingMembers = (collection.memberCount || 0) > members.length ? (collection.memberCount || 0) - members.length : 0;
    ...
    return (
        <div className="animate-in fade-in slide-in-from-right-4">
                ...
                <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6 bg-dark-card border border-dark-border p-8 rounded-2xl relative overflow-hidden">
                    ...
                    <div className="relative z-10 max-w-xl">
                        <div className="flex items-center gap-3 mb-2">
                             ...
                             {isCreator && <button onClick={onUpdateClick} className="p-2 text-gray-500 hover:text-white transition-colors"><Icons.Settings className="w-5 h-5" /></button>}
                        ...
                        <div className="flex flex-wrap gap-2">{collection.tags?.map(tag => <span key={tag} className="px-3 py-1 bg-white/5 border border-white/10 rounded-full text-xs font-mono text-neon-cyan">#{tag}</span>)}</div>
                    </div>
                    ...
                    <div className="relative z-10 flex flex-col gap-4 items-end min-w-[280px]">
                        ...
                        <div className="bg-dark-card/80 backdrop-blur-md border border-dark-border p-5 rounded-xl flex flex-col items-center text-center w-full shadow-xl relative overflow-hidden group">
                           ...
                           {/* Team Avatar Group */}
                           <div className="flex -space-x-6 mb-3 relative z-10 p-1">
                               {members.length === 0 ? (
                                   <div className="relative z-0">
                                     <div className="w-12 h-12 rounded-full ring-2 ring-dark-card bg-gray-800 border-2 border-dark-card flex items-center justify-center">
                                   ...
                                   members.map((m) => (
                                       <div key={m.id} className="relative z-0 hover:z-10 hover:scale-110 transition-transform">
                                           <div className="w-12 h-12 rounded-full overflow-hidden border-2 border-dark-card bg-gray-800 ring-2 ring-dark-card">
                                               {m.avatarUrl ? (
                                                    <img src={m.avatarUrl} alt={m.username} className="w-full h-full object-cover" />
                                               ) : (
                               ...
                               {remainingMembers > 0 && (
                                   <div className="relative z-0 hover:z-10 hover:scale-110 transition-transform">
                                       <div className="w-12 h-12 rounded-full bg-gray-800 text-white border-2 border-dark-card ring-2 ring-dark-card flex items-center justify-center font-bold text-xs">
                                           <span>+{ remainingMembers }</span>
                                       </div>
                                   </div>
                               )}
                        ...
                        <div className="flex gap-2 w-full justify-end">
                            {isCreator && (
                              <Button onClick={() => setShowRequests(true)} variant="secondary" className="relative flex-1">
                                ...
                                {pendingCount > 0 && <span className="absolute -top-1 -right-1 flex h-4 w-4 items-center justify-center rounded-full bg-neon-pink text-[10px] font-bold text-white">{pendingCount}</span>}
                              </Button>
                            )}
            ...
            {loading ? <div className="flex justify-center p-12"><Icons.RefreshCw className="w-8 h-8 animate-spin text-neon-cyan" /></div> : items.length === 0 ? <Empty><EmptyHeader><EmptyMedia variant="icon"><Icons.Image className="w-8 h-8" /></EmptyMedia><EmptyTitle>Empty Collection</EmptyTitle><EmptyDescription>No NFTs have been added to this collection yet.</EmptyDescription></EmptyHeader></Empty> : <Gallery history={items} onSelect={onSelect} selectedId={undefined} title={`${items.length} Items`} />}
            {showRequests && <ManageRequestsModal collectionId={collection.id} onClose={() => setShowRequests(false)} />}
        </div>
    );
};
...
const Collections: React.FC<CollectionsProps> = ({ userId, onRemixCollection, onSelect }) => {
  ...
  const handleViewCollection = (col: Collection) => { setSelectedCollection(col); setView('DETAIL'); };
  const handleBackToList = () => { setSelectedCollection(null); setView('LIST'); };
  const filteredCollections = collections.filter(col => { const q = searchQuery.toLowerCase(); return col.name.toLowerCase().includes(q) || col.tags?.some(tag => tag.toLowerCase().includes(q)); });
  useEffect(() => { setCurrentPage(1); }, [searchQuery]);
  const totalPages = Math.ceil(filteredCollections.length / ITEMS_PER_PAGE);
  const startIndex = (currentPage - 1) * ITEMS_PER_PAGE;
  const currentCollections = filteredCollections.slice(startIndex, startIndex + ITEMS_PER_PAGE);
  const handlePageChange = (page: number) => { if (page > 0 && page <= totalPages) { setCurrentPage(page); window.scrollTo({ top: 0, behavior: 'smooth' }); } };
      ...
      {view === 'LIST' ? (
          <>
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
                <div><h2 className="text-3xl font-black text-white tracking-tighter">Global Collections</h2><p className="text-gray-400">Remix styles from community curated sets.</p></div>
                <div className="flex flex-col sm:flex-row gap-3 w-full md:w-auto"><InputGroup className="w-full md:w-64"><InputGroupInput placeholder="Search collections..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} /><InputGroupAddon><Icons.Search className="w-4 h-4" /></InputGroupAddon></InputGroup><Button onClick={() => setShowCreate(true)} className="bg-neon-cyan text-black hover:bg-white whitespace-nowrap"><Icons.Plus className="w-4 h-4 mr-2" /> Create New</Button></div>
            </div>
            {filteredCollections.length === 0 ? <Empty><EmptyHeader>{searchQuery ? <><EmptyMedia variant="icon"><Icons.Search className="w-8 h-8" /></EmptyMedia><EmptyTitle>No Matches Found</EmptyTitle><EmptyDescription>Try adjusting your search terms.</EmptyDescription></> : <><EmptyMedia variant="icon"><Icons.Box className="w-8 h-8" /></EmptyMedia><EmptyTitle>No Collections Yet</EmptyTitle><EmptyDescription>Be the first to create a themed collection for others to join.</EmptyDescription><EmptyContent><Button onClick={() => setShowCreate(true)}>Create Collection</Button></EmptyContent></>}</EmptyHeader></Empty> : <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">{currentCollections.map(col => <AutoSlideCard key={col.id} collection={col} onRemix={() => onRemixCollection(col)} onView={() => handleViewCollection(col)} userId={userId} />)}</div>}
            {totalPages > 1 && (
              <Pagination className="mb-8">
                <PaginationContent>
                  <PaginationItem><PaginationPrevious onClick={() => handlePageChange(currentPage - 1)} disabled={currentPage === 1} /></PaginationItem>
                  {Array.from({ length: totalPages }, (_, i) => i + 1).map((page) => {
                    if (page === 1 || page === totalPages || (page >= currentPage - 1 && page <= currentPage + 1)) {
                      return <PaginationItem key={page}><PaginationLink onClick={() => handlePageChange(page)} isActive={currentPage === page}>{page}</PaginationLink></PaginationItem>;
                    }
                    if (page === currentPage - 2 || page === currentPage + 2) return <PaginationItem key={page}><PaginationEllipsis /></PaginationItem>;
                    return null;
                  })}
                  <PaginationItem><PaginationNext onClick={() => handlePageChange(currentPage + 1)} disabled={currentPage === totalPages} /></PaginationItem>
                </PaginationContent>
              </Pagination>
            )}
          </>
      ) : ( selectedCollection && <CollectionDetailView collection={selectedCollection} onBack={handleBackToList} onRemix={() => onRemixCollection(selectedCollection)} currentUserId={userId} onUpdateClick={() => setShowEdit(true)} onSelect={onSelect} /> )}
      {showCreate && <CreateCollectionModal userId={userId} onClose={() => setShowCreate(false)} onCreated={() => { loadData(); }} />}
      {showEdit && selectedCollection && selectedCollection.creatorId === userId && <EditCollectionModal collection={selectedCollection} onClose={() => setShowEdit(false)} onUpdated={() => { loadData(); }} />}
    </div>
  );
};
